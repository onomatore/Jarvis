    ...
    def _decode_packet(self, packet: AudioPacket) -> Tuple[AudioPacket, bytes]:
        ...
        if packet:
            try:
                ...
                print(f"OpusError (corrupted stream), len={len(packet.decrypted_data) if packet.decrypted_data else 0}")
                pcm = b''
            return packet, pcm
